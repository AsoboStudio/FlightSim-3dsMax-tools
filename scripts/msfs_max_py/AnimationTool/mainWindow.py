...
rt = pymxs.runtime
import ui.main_view_ui as main_view_ui
...
class MainView(QWidget,main_view_ui.Ui_MainWindow):
    ...
    def __init__(self):
        ...
        self.right_suffix = self.rightSuffixLbl.text()
        self.left_suffix = self.leftSuffixLbl.text()

        global LOGGER
        LOGGER = self.logger

        self.bakeAnimationBtn.clicked.connect(self.on_bake_animation_clicked)
        self.mirrorSkinBtn.clicked.connect(self.on_mirror_skin_clicked)
        self.toggleMirrorDirection.clicked.connect(self.on_toogledir_clicked)
        self.mirrorObjectBtn.clicked.connect(self.on_mirror_hierarchy_without_skin_clicked)
        self.resetScaleBtn.clicked.connect(self.on_reset_scale_clicked)
        self.mirrorAnimationBtn.clicked.connect(self.on_mirror_animation_btn_click)
        self.leftSuffixLbl.textChanged.connect(self.on_left_text_changed)
        self.rightSuffixLbl.textChanged.connect(self.on_right_text_changed)
    ...
    def on_toogledir_clicked(self):
        if self.copy_direction == CopyDirection.LEFT_TO_RIGHT:
            ...
            self.toggleLabel.setText("Right To Left")
            return
        if self.copy_direction == CopyDirection.RIGHT_TO_LEFT:
            self.copy_direction = CopyDirection.LEFT_TO_RIGHT
            self.toggleLabel.setText("Left To Right")
            return
